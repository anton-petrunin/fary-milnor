 \section{Ekholm, White, and Wienholtz}

In this section we discuss a solution of the problem based a theorem of Tobias Ekholm, Brian White, and Daniel Wienholtz \cite{EWW_embed}.
This theorem was a breakthrough in minimal surface theory at the time.
At the same time it was based on an elementary idea that we are going to explain.

We start with a polygonal curve $\alpha$ with total curvature less than $4\cdot\pi$;
show that area-minimizing disc spanned by $\alpha$ has no self-intersections, and therefore $\alpha$ has to be a trivial knot.
So in a way the equation for area-minimizing surface solves our problem, we only need to understand it.

The main hiero in this proof is the so-called \emph{extended monotonicity theorem}.
We will also apply the \emph{Doublas--Rado theorem} on the existence of area-minimizing disc and reuse the inequality between total curvature and angular length from the F\'ary's proof.
...
The following statement about area-minimizing discs is easy to believe, but not easy to prove. %???REFS

\begin{thm}{Doublas--Rado theorem}\label{thm:min-exists}
Given a simple closed polygonal curve $\alpha$ in $\mathbb{R}^3$, there is a area-minimizing disc $\Sigma$ with boundary line $\alpha$; it is a smooth surface, possibly with self-intersections and isolated singularities.
...
$\lim_{r\to\infty}\frac{W_o(r)}{r^2}=\tfrac12\cdot \Psi_o(\alpha)$, where $\Psi_o(\alpha)$ denotes angular length of $\alpha$ with respect to $o$; see Section~\ref{sec:fary}.
\end{subthm}
...
The classical monotonicity theorem states that the same holds for balls that do not touch the boundary of surface.
The stated version is due to Brian White \cite{white}.
The same statement holds for minimal surfaces \cite{EWW_embed}; its proof requires dipper diving into differential geometry.
At the same time, the original formulation admits generalization to wider class of ambient spaces \cite{St_structure}.
...
Note that $\Delta_r'$ differ from $\Delta_r$ only inside $\Sigma$.
...
In general, the arguments shows that the absolute continuous part of $W_o(r)/r^2$ is nondecreasing which suffices.
...
It follows that $W_o(r)/r^2$ approaches the area of unit ball in this cone as $r\to\infty$ --- hence the result.
...
the main part of theorem implies that $W_o(r)\ge\pi\cdot r^2$ for \emph{any} point $o\in\Sigma$ --- hence the result.
\qeds
