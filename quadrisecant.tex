...
The question about existence of a quadrisecant was asked by Otto Toep\-litz and answered by Erika Pannwitz for tame knots in \emph{general position} \cite{pannwitz}.
...
This statement can be deduced from  the \textit{loop theorem} --- a heavy weapon of 3-dimensional topology based on so-called \textit{Dehn's lemma} proved by Christos Papakyriakopoulos.
...
If $(x,y)\in C_3$ and $r(x,y)$ crosses the first arc, then we write $(x,y)\in C_3^+$;
