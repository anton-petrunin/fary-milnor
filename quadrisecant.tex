\section{Pannwitz and Hopf}\label{sec:quadrisecant}
...
what is imprtant is that any knot is arbitrary close to a knot in general position.
...
Then $acbd$ is an inscribed quadrileteral with all external angles equal to $\pi$.
...
In the proof of proposition, we will use the following characterization of trivial knot.
...
For those who familiar with Dehn's lemma, it would be an exercise; otherwise, we suggest to take it as granted.
In the following proof we follow closely presentation of Carsten Schmitz \cite{schmitz} and the original argument of Erika Pannwitz.
...
Color the set formed by union of the squares that intersect $\bar C_3^+$ or lie in the lowest row of squares for $y\le \tfrac1n$.
Note that we did not color squares in the upper row and the boundary of colored set contains a simple curve $t\mapsto(x(t),y(t))$ that cuts  the cylinder $\mathbb{S}^1\times (0,1)$ into two cylinders.
...
This way we obtain a piecewise linear annulus that connects the curve $t\mapsto \alpha(x(t))$ to a curve on the surface of cube.
